...
The ensemble calculates a weighted sum of individual model probabilities.

### Majority-Vote Safeguard
To prevent false-positives against high-quality real photographs, the system enforces a strict voting safeguard:
- A model is considered to "vote real" if its AI probability is `< 50%`.
- If at least 2 out of the 3 primary models (â‰¥60%) vote real, the final score will be hard-capped at 45.0% (Real/Uncertain range), regardless of any single hallucinated high-AI probability by the third model.
- If at least 2 out of the 3 vote AI (â‰¥60%), the score is safely boosted.
